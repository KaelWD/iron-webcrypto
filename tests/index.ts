...
export const tests = ({
  ...
  describe('Iron', () => {
    ...
    it('fails to turn object into a ticket (failed to stringify object)', async () => {
      ...
      cyclic[0] = cyclic
      const key = Iron.randomBits(128)
      await rejects(Iron.seal(cyclic, key, Iron.defaults), [
    ...
    describe('generateKey()', () => {
      it('returns an error when password is missing', async () => {
        // @ts-expect-error
        await rejects(Iron.generateKey(null, null), 'Empty password')
      })
      ...
      it('returns an error when options are missing', async () => {
        // @ts-expect-error
        await rejects(Iron.generateKey(password, null), 'Bad options')
        // @ts-expect-error
        await rejects(Iron.generateKey(password, 'abc'), 'Bad options')
      })
      ...
      it('returns an error when invalid salt bits are provided', async () => {
        ...
        await rejects(Iron.generateKey(password, options), [
    ...
    describe('encrypt()', () => {
      it('returns an error when password is missing', async () => {
        // @ts-expect-error
        await rejects(Iron.encrypt(null, null, 'data'), 'Empty password')
      })
    })
    ...
    describe('decrypt', () => {
      it('returns an error when password is missing', async () => {
        // @ts-expect-error
        await rejects(Iron.decrypt(null, null, 'data'), 'Empty password')
      })
    })
    ...
    describe('hmacWithPassword()', () => {
      it('returns an error when password is missing', async () => {
        // @ts-expect-error
        await rejects(Iron.hmacWithPassword(null, null, 'data'), 'Empty password')
      })
    ...
    describe('seal()', () => {
      it('returns an error when password is missing', async () => {
        // @ts-expect-error
        await rejects(Iron.seal('data', null, Iron.defaults), 'Empty password')
      })
    ...
    describe('unseal()', () => {
      ...
      it('returns an error when password is missing', async () => {
        ...
        // @ts-expect-error
        await rejects(Iron.unseal(ticket, null, Iron.defaults), 'Empty password')
      })
      ...
      it('returns an error when decrypted object is invalid', async () => {
        ...
        const ticket = `${macBaseString}*${mac.salt}*${mac.digest}`
        await rejects(Iron.unseal(ticket, password, Iron.defaults), [
